...
const ProjectCard = ({ project: { title, description, tags, link } }) => {
  return (
    <div className="group w-full sm:w-1/2 m-4 mx-auto p-6 rounded-xl border-2 border-gray-800">
      <a href={link}>
      ...
      <hr className="my-4 border-1 border-gray-800" />
      <p className="text-center">{description}</p>
      <div className="mt-4 mb-8 flex flex-wrap justify-center items-center gap-2">
        {tags.map((tag) => (
          <div className="px-4 py-1 border-2 border-gray-800 rounded-full">{tag}</div>
        ))}
      </div>
